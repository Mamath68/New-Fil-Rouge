<?php
include('Partiel/header.php');

echo "<title>Instruments à Percussions - Les Membraphones</title>";
?>

<br>
<div class="container">
    <p>Il existe 3 catégories d'Instruments à Percussions :
    <ol class="instruments">
        <li class="new-font">Les Membraphones, dont nous allons aborder le sujet sur cette page</li>
        <li class="new-font">Les Idiophones</li>
        <li class="new-font">Les électrophones</li>
    </ol>
    </p>
</div>
<br>
<p>Un membranophone est un instrument de percussion dont les sons sont produits par la vibration d'une membrane tendue sur un cadre.
    <br>
    La membrane peut être frappée par une main comme sur un djembé, par un instrument (baguettes, balais, etc.), comme sur la caisse claire.
    <br>
    Elle peut aussi entrer en vibration par le frottement d'une tige solidaire de la peau tendue sur un fût résonnant comme la cuica (tambour à friction). Le tambour par exemple, est un membranophone
</p>
<br>
<caption>
    <p class="text-center caption bg-black">Les Membraphones</p>
</caption>
<table class="table container">
    <thead>
        <!--tr = Lignes-->
        <tr class="table-dark text-center tableau">
            <!--th = Header-->
            <th scope="col">Instruments</th>
            <th scope="col">Images</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <!--td = Cellule-->
            <td class="table-dark text-center tableau">Alfaia</td>
            <td class="text-center"><img src="Images/Ailfaia.png" alt="Alfaia"></td>
        </tr>
        <tr>
            <td class="table-dark text-center tableau">Tama</td>
            <td class="text-center"><img src="Images/Tama.png" alt="Tama"></td>
        </tr>
        <tr>
            <td class="table-dark text-center tableau">Bombo</td>
            <td class="text-center"><img src="Images/Bombos.png" alt="Bombos"></td>
        </tr>
        <tr>
            <td class="table-dark text-center tableau">Tam-Tam</td>
            <td class="text-center"><img src="Images/Tam_tam.png" alt="Tam-Tam"></td>
        </tr>
    </tbody>


</table>

<div class="fixed">
    <?php include('Partiel/footer.php') ?>
</div>